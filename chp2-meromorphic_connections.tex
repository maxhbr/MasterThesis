...
\begin{defn}\label{defn:elemnMerConnBausteine}
  \begin{itemize}
    \item For a $\phi\in\C(\!\{t\}\!)$ we use $\cE^{\phi}$ to denote the
      ...
      \begin{s-prop}\label{cor:uniqueAmbassadorForPhi}
        ...
        will only assume, that $\phi$ is always given by the unique ambassador
        which has no holomorphic part.
      \end{s-prop}
    \item For $\alpha\in\C$, define the \emph{elementary regular meromorphic
      connection of rank one} $\cN_{\alpha,0}$ as the germ
      \[
        (\cN_{\alpha,0},\nabla)\cong\left(\C(\!\{t\}\!),d+\frac{\alpha}{t}\right)
        \,.
      \]
