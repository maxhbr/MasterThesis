...
\iffalse
  ...
  %       number, i.e.\ $q_j \underset{\tilde\theta}{\prec} q_l$ and
  %       $\Im(a_{jl}e^{-ik_{jl}\theta})=0$.
  %         \item[\Leftrightarrow{}] $e^{(q_j-g_l)(t^{-1})}$ is of maximal decay in
...
\begin{lem}\label{lem:rotationalSym}%\label{rem:rotationalSymPrime}
  ...
  of degree $k$ is given, we have for every $l\in\N$ that
  \[
    \theta+l\frac{\pi}{k} \in \A \,.
  \]
  \begin{s-cor}
...
\end{defn}
Let us introduce a couple of notations, which coincide with the notations
used in Loday-Richaud's paper \cite{Loday1994}.
\begin{notations}
...
\begin{proof}
  \begin{comment}
    \begin{enumerate}
      \item \TODO[clear, since germwise clear?]
    \end{enumerate}
  \end{comment}
  \begin{comment}
    \begin{enumerate}
      \setcounter{enumi}{1}
      \item
        \TODO[Germwise or sectionwise??, check only on generators?]
