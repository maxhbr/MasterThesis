...
are also useful, especially in the sections~\ref{sec:StokesGroup}
and~\ref{sec:mainThm2}.
Also useful was \rewrite{Boalch's} paper \cite{boalch} (resp.\ his thesis
\cite{thboalch}) which looks only at the single leveled case.

Let $\cM^{nf}$ be a fixed model with the corresponding connection matrix $A^0$.
...
is isomorphic, via $\exp$, to the first cohomology
...
The morphism, which maps each Stokes cocyle to its corresponding $1$-cocycle
...
chapter~\ref{chap:multisummability}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Stokes structures: Malgrange-Sibuya isomorphism}\label{sec:mainThm1}
\marginnote{\cite[Thm.I.2.1]{Loday1994}, \cite[Thm.4.3.9]{Loday2014},
...
There exists an open covering $\cU=(U_j)_{j\in J}$ and for every open set $U_j$
...
\begin{rem}\label{rem:expNonNormalForm}
  \begin{s-defn}
    ...
    $\exp_{A^0}=\exp$\footnote{In Loday's book~\cite{Loday1994} this is denoted
    as $\exp_{\mu_0}$.}.
  \end{s-defn}
  \begin{enumerate}
...
\begin{proof}
  ...
  The \textbf{surcjectivity} can also \rewrite{be seen easily}, since every
