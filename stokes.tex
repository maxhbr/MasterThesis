...
\begin{center}
  \begin{tikzpicture}[scale=3]
    ...
    \node[green!40!black] at (1,0.7) {\TODO[\cite{sabbah2007isomonodromic}]};
    \node[blue] at (2.3,-1.3) {\TODO[\cite{thboalch},\cite{boalch}]};
    \node[purple] at (4,0)
    {\TODO[\cite{Loday1994}{\footnotesize(\cite{Loday2014})}]};
  \end{tikzpicture}
\end{center}
...
\begin{rem}
  ...
  this in the single-leveled case by \rewrite{calling our Stokes matrices
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Which information is required to describe a Stokes germ?}
...
\rewrite{First} we want to use the faithful matrix representations of Stokes
germs, discussed in section~\ref{sec:matrixReps}, to \rewrite{obtain some
statements} about the \rewrite{structure} of the Stokes germs in our situation.
...
\end{enumerate}
In the next section, we will use this decompositions in the other direction, to
...
\begin{prop}\label{prop:result2}
  Similar to proposition~\ref{prop:result1} we can also define the isomorphism
  \[
    \C^{k_1+2\cdot k_2}\longrightarrow\St(A^0)
  \]
  which makes the following diagram commute.
  \[ \begin{tikzcd}[column sep=2cm,row sep=3cm]
      &\C^{k_1+2\cdot k_2} 
      \dlar[blue]{\prod_{\theta\in\A}\rho_{\tilde\theta}^{-1}\circ\bar\vartheta_\theta}
      \arrow[dr,green!50!black]
    \\\prod_{\theta\in\A}\Sto_\theta(A^0)
      \arrow[rr,purple,"h"]
      &&\St(A^0)
  \end{tikzcd} \]
...
\[ \begin{tikzcd}
  ...
  \\&\prod_{k\in\{k_1,k_2\}}\prod_{\theta\in\A}\SSto_{\theta}^{k}(A^0)
     \arrow[dl,blue]
     \rar[green!50!black]
    &\prod_{k\in\{k_1,k_2\}}\Sto_{\theta}^{k}(A^0)
     \dar[green!50!black]{\prod_{k\in\{k_1,k_2\}}i^k}
  \\\prod_{\theta\in\A} \SSto_{\theta}(A^0)
    \dar[blue]{\prod_{\theta\in\A}\rho_{\tilde\theta}^{-1}}
    && 
    \prod_{k\in\cK}\Gamma(\dot\cU^k;\Lambda^k(A^0))
    \dar[green!50!black]{\cT}
  \\\prod_{\theta\in\A} \Sto_{\theta}(A^0)
    \arrow[rr,purple,"h"]
    && \St(A^0)
\end{tikzcd} \]
\begin{comment}
  To define this isomorphism we use the steps one and two \rewrite{from the
  definition of the isomorphism in proposition}~\ref{prop:result1}.
\end{comment}
