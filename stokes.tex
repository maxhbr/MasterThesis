...
\begin{defn} \label{defn:StokesSheaf}
  ...
  \end{s-rem}
  \begin{s-rem}
    \TODO[remove? need more defs]
    Sabbah \cite[110]{sabbah2007isomonodromic} talks (global) about meromorphic
...
\begin{proof}[Proof of the injectivity]
  ...
  \[
    F_l'F_j'^{-1}=G_lF_lF_j^{-1}G_j^{-1}
    \text{~on~the~arc~} U_j\cap U_l \,.
  \]
  If we set $H_j:=F_j^{-1}G_{j}^{-1}F_j'$
  \comm{($H_j^{-1}:=F_j'G_{j}F_j^{-1}$)} on $U_{j}$, we get
  \begin{itemize}
    \item a solution of $[A,A']$\TODO[Proofe!!!!], i.e.\ satisfies
      ${}^{H_j}A=A'$,
      \begin{comment}
        \begin{align*}
          {}^{H_j}A &= (dH_j)H_j^{-1}+H_jAH_j^{-1}
          \\&=(d(F_j^{-1}G_{j}^{-1}F_j'))F_j'G_{j}F_j^{-1}
              F_j^{-1}G_{j}^{-1}F_j'AF_j'G_{j}F_j^{-1}
        \end{align*}
      \end{comment}
    \item which satisfies $\hat F'=\hat H_j^{-1}\hat F$ on every $U_j$, since
      for every $j\in J$ is $G_j$ flat.
  \end{itemize}
...
\[
  \mathcal{G}(I)=\left\{g\in\Gl_n(\cA(I))\mid g\sim_I\id\right\}\,,
\]
and this defines a sheaf $\mathcal{G}$ on $S^1$.
The statement of the Malgrange-Sibuya Theorem is then, that the
\rewrite{differece} between formal and konvergent invertible matrices is
described by the first sheaf cohomology $H^1(S^1,\mathcal{G})$ of
$\mathcal{G}$ via the map
\[
  \Theta: G\llbracket t\rrbracket/G\{t\}\to H^1(S^1,\mathcal{G}) \,,
\]
...
\begin{einr}
  ...
  By $(F_lF_j^{-1})_{j,l\in J}$ is then a cocycle for $\mathcal{G}$ defined,
...
\begin{itemize}
  \item the choise of an ambassador $\hat F$ in
    $[\hat F]\in G\llbracket t\rrbracket/G\{t\}$\TODO[proof!],
  \item the choice of the covering $\cU$\TODO[proof!] nor
  \item the choice the $F_j$\TODO[proof!].
\end{itemize}
\begin{lem}
...
\begin{proof}
  ...
  $g\sim\id$.
  \TODO[Riemannscher hebbarkeits satz]
  This defined $g$ satisfies $g=F'^{-1}F$, so that $[F]=[F']$.
\end{proof}
\TODO[\cite{babbitt1989local} on page 66]
\begin{thm}[Malgrange-Sibuya]
  The map $\Theta$ is an isomorphism
  \[
    G\llbracket t\rrbracket/G\{t\}\cong H^1(S^1,\mathcal{G}).
  \]
  \begin{s-rem}
    Since we know that
    ...
    we get also
    \[
      G(\!(t)\!)/G(\!\{t\}\!)\cong H^1(S^1,\mathcal{G}).
    \]
  \end{s-rem}
\end{thm}
...
\begin{proof}[Proof of surjectivity]
  ...
  $H^1(S^1,\Gl_n(\cA^{<0}))$.
  We then can use the Theorem 4.2.1{} in Babbitt and Varadarajans's book
  \cite[65ff]{babbitt1989local}, which states the following:
  \begin{thm}[Malgrange-Sibuya]
    There is a isomorphism
    \[
      G\llbracket t\rrbracket/G\{t\}\cong H^1(S^1,\Gl_n(\cA^{<0})).
    \]
  \end{thm}
  We also know that
  $G\llbracket t\rrbracket/G\{t\}\cong G(\!(t)\!)/G(\!\{t\}\!)$\TODO[(cf~??)],
  thus we can use $G(\!(t)\!)/G(\!\{t\}\!)\cong H^1(S^1,\Gl_n(\cA^{<0}))$.
  \TODO[lese weiter bei babbitt1989local]
\end{proof}
...
\begin{proof}[Proof of surjectivity]
  ...
  part of the proof by giving a necessary and sufficient condition for a class
  $\lambda$ in $H^1(S^1,\Lambda(A^0))$
...
\begin{prop}\label{prop:representation}
  ...
  \end{align*}
  is an isomorphism which maps a germ of $\Sto_\theta(A^0)$ to the
  corresponding Stokes matrix $C_{\cY_{0,\tilde\theta}}$ such that
  \begin{equation} \label{eq:representation}
    \phi_\theta(t)\cY_{0,\tilde\theta}(t)
    =\cY_{0,\tilde\theta}(t)C_{\cY_{0,\tilde\theta}}
  \end{equation}
  near $\theta$.
  The matrix $C_{\cY_{0,\tilde\theta}}$ is a representation of $\phi_\theta$.
  \begin{s-rem}
    \begin{enumerate}
      ...
      %     \,.
      %   \end{align*}
      \item In Loday's book \cite[78]{Loday2014} are the elements of
        ...
        constant invertible matrix $C_{\cY_{0,\tilde\theta}}$.
    \end{enumerate}
  \end{s-rem}
\end{prop}
\begin{proof}
  In general, the map $\rho_{\tilde\theta}$ maps to the group $G$ of constant
  matrices, since \rewrite{conjugation} by the fundamental solution yields a
  constant matrix.
  ...
  It is \textbf{injective}, and thus a faithful representation, since
  conjugation by an invertible matrix defines an isomorphism to its
  \rewrite{image}.
...
\begin{comment}
  ...
  an unique $1$-cocycle, which is a Stokes cocycle.
\end{comment}
...
\begin{proof}
  \begin{enumerate}
    \item This statement is clear by construction.
...
\begin{proof}
  \begin{enumerate}
    \item The map
      ...
      arc of definition and thus a group isomorphism. \TODO[why?]
    \item The second map
      \[
        s^k:
...
\begin{defn}\label{defn:theMapTau}
  ...
  \[
    \dot F_\theta=\begin{cases}
      ...
      & \text{~when } \theta\in\A^k
    \\1 \text{~(the identity on $\dot U_\theta$) }
      & \text{~when } \theta\notin\A^k
    \end{cases} \,.
  \]
  ...
  \begin{s-rem}
    \begin{enumerate}
      \item This map, again, is injective and
        \TODO[proof?]
...
\begin{einr}
  $\left(f_\theta\right)_{\theta\in\A}$ where
  $f_\theta:\cU_\theta^k\to\C$\TODO[ is \textbf{holomorphic}/meromorphic?].
\end{einr}
...
\TODO[move to single leveled case?]
\begin{comment}
