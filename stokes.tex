...
\begin{cor}
  ...
  $\dim_\C\St(A^0)=\sum_{\alpha\in\A}\dim_\C\SSto_\alpha(A^0)$ and
  $\dim_\C\SSto_\alpha(A^0)=
  \sum_{q_j\underset{\alpha,\max}{\prec}q_l}\deg(q_j-q_l)$
...
\begin{proof}[Proof of theorem~\ref{thm:mainThm2}]
  ...
  \end{tikzcd} \]
  by $\mathfrak{T}$ with the result that we obtain the bijection
  \[
    \cT\circ\mathfrak{T}:
    \prod_{\alpha\in\A}\Sto_\alpha(A^0)
    \longrightarrow
...
\begin{lem}
  ...
  \[
    K\in S_T:=\left\{K=(K_{jl})_{j,l\in\{1,\dots,n\}}\in\GL_n(\C)\mid
...
\begin{thm}
  ...
  $\theta\pm\frac{\pi}{2k}\notin\A^k$, then
  \[
    \cH\cong \prod_{k\in\cK} \prod_{j\in\{1,\dots,2k\}}
  ...
  \begin{align*}
    ...
    \\&= \Big\{K=(K_{jl})_{j,l\in\{1,\dots,s\}}\in\GL_n(\C)\mid
      K_{jl}=\delta_{jl} \text{~unless~}
      q_j \underset{\theta}{\prec} q_l \Big\}
  \end{align*}
  \begin{s-rem}
    This means that the information of all Stokes-matrices can on every level
    $k$ be grouped into $2k$ matrices, which are products of the corresponding
    Stokes matrices.
  \end{s-rem}
\end{thm}
\begin{proof}
  Let $I=U(\theta,\frac{\pi}{k})$ be the arc of width $\frac{\pi}{k}$
  ...
  \begin{itemize}
    \item for every $\alpha\in\A$ satisfies the set
      ...
      relation}, and
    \item the union of all $T_{\alpha}$ for $\alpha\in I\cap \A^k$ is then
      \begin{align*}
  ...
  \end{itemize}
  Thus, by Corollary~\ref{cor:composeLevelwise} we have an isomorphism
  \begin{align*}
    \prod_{\alpha\in\A^k\cap I}
    \overset{S_{T_\alpha}}{%
      \overset{\text{\rotatebox[origin=c]{-90}{$=$}}}{\overbrace{%
          \SSto_\alpha^k(A^0)
        }
      }
    }
    \overset{\cong}\longrightarrow
    S_T=:\hat\SSto_\theta^k(A^0)
  \end{align*}
  ...
  \[
    ...
    \hat\SSto_{\theta+j\frac{\pi}{k}}^k(A^0) \,.
  \]
  \rewrite{The product for every $k\in\cK$ of this isomorphism yields the
  isomorphisms of the theorem.}
\end{proof}
