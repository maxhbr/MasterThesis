...
open set $U_j$ a lift $F_j\in\Gl_n(\cA(U_j))$, which solves $[A,A^0]$.  By
$(F_kF_j^{-1})_{jk}$ is then a cocycle of the sheaf $\St(A^0)$, relative to the
covering $\cU$, defined.
...
\begin{proof}[Proof of the injectivity]
  ...
  \[
    F_l'F_j'^{-1}=G_lF_lF_j^{-1}G_j^{-1} \text{ on } U_j\cap U_l.
  \]
  If we set $H_j:=F_j^{-1}G_{j}^{-1}F_j'$
  \comm{$H_j^{-1}:=F_j'G_{j}F_j^{-1}$} on $U_{j}$, we get a solution
  of $[A,A']$\TODO[Proofe!!!!] which satisfies $\hat F'=H_j^{-1}\hat F$ on
  every $U_j$.  Therefore are $(A,\hat F)$ and $(A,\hat F')$ equivalent and
