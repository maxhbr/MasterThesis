...
\begin{defn}
  Let $\theta\in\A$ and $k\in\cK_\theta$. An arc
  $\mathfrak{s}_{\theta-\frac{\pi}{2k},\theta+\frac{\pi}{2k}}$, which is (for a
  sufficiently small radius) a sector with opening $\frac{\pi}{k}$ and is
  bisected by $\theta$, is called a \emph{Stokes arc of level $k$ at $\theta$}.
\end{defn}
