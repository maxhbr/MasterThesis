    ...
    \item $\theta$ is an \emph{anti-Stokes direction} if there is at least one
      ...
      \begin{itemize}
        \item Let $\A=\{\theta_1,\dots,\theta_{\nu}\}$ denote the set of all
          ...
          \PROBLEM[make all Stokes dirs $\alpha$ instead of $\theta$?]
          \begin{s-rem}
...
\begin{lem}\label{lem:rotationalSym}%\label{rem:rotationalSymPrime}
  If $\theta\in\A$ together with a pair $(q_j-q_l)(t^{-1})\in\cQ(A^0)$
...
\begin{defn}\label{defn:groupOfFaithfullReps}
  ...
  \begin{s-rem}
    ...
    Let
    \begin{itemize}
      \item $q_1 = 1/t^k + t/t^k$,
      \item $q_2 = 2/t^k + t/t^k$ and
      \item $q_3 = 1/t^k + 2t/t^k$
    \end{itemize}
    be the three diagonal elements of $Q$ of some system $A$.
    Thus the determining polynomials are given by
    \begin{itemize}
      \item $q_{1,2}=q_1-q_2 = 1/t^k$,
      \item $q_{1,3}=q_1-q_3 = -t/t^k$ and
      \item $q_{2,3}=q_2-q_3 = 1/t^k -t/t^k$.
    \end{itemize}
    Since the leading terms of $q_{1,2}$ and $q_{2,3}$ are equal, they
    determine the same anti-Stokes directions. Let $\theta$ be one of the
    directions, which does neither satisfy 
  ...
  \begin{enumerate}
    \item For any $k\in\cK$, there are the two following ways of factoring
      ...
      \begin{s-rem}\label{rem:algFactorization}
        ...
        $1+\sum C_{\tilde\theta}^{(j,l)}$ of $f^{\leq k}$ only the blocks
        $C_{\tilde\theta}^{(j,l)}$ such that $\deg(q_j-q_l)<k$.
...
\begin{prop}\label{prop:filtrationOfStokesGroup}
  ...
  (cf.\ \cite[Prop.I.5.5]{Loday1994}). Thus, for $\theta\in\A$, one has
  \[
