...
Is is easy to translate the Malgrange-Sibuya isomorphism to the language of
systems.
\begin{comment}
  \begin{itemize}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item $\boldmath q_j \underset{\tilde\theta,\max}{\prec} q_l$
      ...
      (where $k$ is the degree of $q_j-q_l$).
      ...
      \begin{s-rem} \label{rem:rotationalSym}
        ...
        $q_j \underset{\tilde\theta,\max}{\prec} q_l$, i.e.\ let
        $a_{jl}e^{-ik\tilde\theta}\in\R_{<0}$.
        Hence, for $n\in\N$,
        \begin{align*}
        ...
        \[
          \begin{cases}
            q_j \underset{\tilde\theta+n\frac{\pi}{k},\max}{\prec} q_l
            & \text{when $n$ is even}
          \\q_l \underset{\tilde\theta+n\frac{\pi}{k},\max}{\prec} q_j
...
\begin{defn}
  ...
  \begin{enumerate}
    \item $\theta$ is an \emph{anti-Stokes direction} if there is at least one
      ...
      \end{itemize}
      \begin{s-rem}
        From remark~\ref{rem:rotationalSym} follows, that in the case
        $\cK=\{k\}$, the set $\cA$ has $\frac{\pi}{k}$-rotational symmetry.
      \end{s-rem}
