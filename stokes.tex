...
\begin{prop}\label{prop:representation}
  ...
  \begin{align*}
    \rho_{\tilde\theta}:\Sto_\theta(A^0)&\longrightarrow\SSto_\theta(A^0)
    \\\phi_\theta
    &\longmapsto
    C_{\cY_{0,\tilde\theta}}
    :=1+C_{\tilde\theta}
...
\begin{rem}\label{rem:mapStoToCocy}
  ...
  \[
    \prod_{\alpha\in J}\Sto_\alpha(A^0)
    \longrightarrow
    H^1(S^1;\Lambda(A^0))=\St(A^0)
  \]
  is welldefined.
...
\begin{defn}\label{defn:stokesCocycle}
  ...
  \prod_{j\in\{1,\dots,\nu\}}\Gamma(U_j;\Lambda(A^0))$
  $\Lambda(A^0)$ corresponding to some cyclic covering with nerve
  $\dot\cU=\{\dot U_j\mid j\in\{1,\dots,\nu\}\}$, which satisfies for every $j$
  \begin{itemize}
...
\begin{defn}
  ...
  \[ \begin{tikzcd}
      \Gamma(\cU;\Lambda^\star(A^0)) \rar& H^1(S^1;\Lambda^\star(A^0))
  \end{tikzcd} \]
  ...
  \begin{enumerate}
    \item The first map is defined as
      ...
      \\\dot f=(\dot f_\theta)_{\theta\in\A^k}
        &\longmapsto (\dot G_\theta)_{\theta\in\A^{\leq k}}
      \end{align*}
      where
      \begin{itemize}
    ...
    \item and the second map
      \begin{align*}
        ...
        &\longrightarrow \Gamma(\dot\cU^{\leq k};\Lambda^{\leq k}(A^0))
      \\\dot f=(\dot f_\theta)_{\theta\in\A^{<k}}
        &\longmapsto (\dot F_\theta)_{\theta\in\A^{\leq k}}
      \end{align*}
