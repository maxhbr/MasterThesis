...
\begin{prop}
  ...
  In this situation there is an isomorphism
  \[
    \rho_{\tilde\theta}:\Sto_\theta(A^0)\to\SSto_\theta(A^0)
  \]
  which maps a germ of $\Sto_\theta(A^0)$ to the unique matrix
  $1+C_{\tilde\theta}$ such that 
...
After decomposing \TODO[Fitting the structure of $Q$] $C_{\tilde\theta}$ into
\begin{align*}
...
\end{align*}
where the $c_{(j,l)}$ are blocks of size $r_j\times r_l$ which correspond to
...
\begin{lem}
  ...
  \[
    ...
    C_{\tilde\theta}^{(l,j)}
  \]
  and $C_{\tilde\theta}^{(l,j)}$ has the necessary block structure.
\end{lem}
...
\[
  \cK(\phi_\theta):= \{\deg(q_j-q_l)\mid C_{\tilde\theta}^{(l,j)}\neq0
    \text{ in some representation of }\phi_\theta\}
\]
\begin{defn}
...
\begin{defn}
  ...
  One also defines
  \begin{itemize}
    \item $\cU^{k}$ \emph{adequate} to $\Lambda^{k}(A^0)$,
    \item $\cU^{\leq k}$ \emph{adequate} to $\Lambda^{\leq k}(A^0)$ and
    \item $\cU^{<k}$ \emph{adequate} to $\Lambda^{<k}(A^0)$
  \end{itemize}
  in a similar way.
\end{defn}
...
\begin{prop}
  ...
  \begin{defn}
    An arc $\mathfrak{s}_{\theta-\frac{\pi}{2k},\theta+\frac{\pi}{2k}}$, which
    is (for a sufficiently small radius) a sector with opening $\frac{\pi}{k}$
    bisected by $\theta$, is called a \emph{Stokes arc of level $k$ at
    $\theta$}.
  \end{defn}
