...
Instead of $q_j-q_l\in\cQ(A^0)$ we will talk of (ordered) pairs
$(q_j,q_l)\in\cQ(A^0)$.
...
\begin{defn}
  ...
  \emph{maximal decay in a direction $\tilde\theta\in
  S^1$}\PROBLEM[determination is not in $S^1$?] if and only if
  $-ae^{-ik\tilde\theta}$ is real negative.
\end{defn}
...
\begin{rem}\label{rem:relationDistanceCondition}
  ...
  \[
    q_j \underset{\tilde\theta}{\prec} q_l
  \]
  is equivalent to
  \begin{einr}
    there is a $\theta'\in U(\tilde\theta,\frac{\pi}{k})$ such that
...
\begin{proof}
  ...
  Let $\alpha\in S^1$ with determination $\tilde\alpha$.
  Let $(j,l)$ be a pair such that
  $q_j \underset{\tilde\alpha,\max}{\prec} q_l$, i.e.\ such that
  $a_{jl}e^{-ik_{jl}\tilde\alpha}\in\R_{<0}$.
  Hence, for $m\in\N$,
  \begin{align*}
    a_{jl}e^{-ik_{jl}\left(\tilde\alpha+m\frac{\pi}{k_{jl}}\right)}
    &=a_{jl}e^{-ik_{jl}\tilde\alpha}e^{-im\pi}
    = \begin{cases}
      a_{jl}e^{-ik_{jl}\tilde\alpha}\in\R_{<0}
        & \text{, if $m$ is even}
    \\-a_{jl}e^{-ik_{jl}\tilde\alpha}\in\R_{>0}
  ...
  obtain
  $a_{lj}e^{-ik_{lj}\left(\tilde\alpha+m\frac{\pi}{k_{lj}}\right)} \in\R_{<0}$.

  Thus, for $\tilde\alpha':=\tilde\alpha+m\frac{\pi}{k_{jl}}$ as determination
  of $\alpha':=\alpha+m\frac{\pi}{k_{jl}}$, we have $\alpha'\in\A$ since
  \begin{itemize}
    \item $q_j \underset{\tilde\alpha',\max}{\prec} q_l$ when $m$ is even or
    \item $q_l \underset{\tilde\alpha',\max}{\prec} q_j$ when $m$ is uneven.
  \end{itemize}
\end{proof}
...
For every determination $\tilde\theta$ of $\theta\in S^1$ we get a function
$e^{Q(1/|t|e^{i\tilde\theta})}$ as a realization of the formal exponential
$e^{Q(1/t)}$.
  ...
  \begin{s-rem}
    \begin{enumerate}
      ...
      \item \marginnote{\cite[Defn.I.4.7]{Loday1994}}
        ...
        $\phi_\theta\in\Lambda_\theta(A^0)\supsetneq\Sto_\theta(A^0)$ relative
        to $\cY_{0,\tilde\theta}$ into its unique representation matrix
        \[
          C_{\cY_{0,\tilde\theta}}
          \in\left\{K\in G\mid K_{jl}=\delta_{jl} \text{ unless }
          q_j \underset{\theta}{\prec} q_l \right\}
          \supsetneq\SSto_\theta(A^0) \,.
        \]
    \end{enumerate}
  \end{s-rem}
\end{prop}
...
\begin{cor}
  ...
  \begin{einr}
    ...
    only if there exists a $C_{\cY_{0,\tilde\theta}}\in\SSto_\theta(A^0)$ such
    that $\phi_\theta=\cY_{0,\tilde\theta}C_{\cY_{0,\tilde\theta}}
    \cY_{0,\tilde\theta}^{-1}$ in some representation $\tilde\theta$ of
    $\theta$.
  \end{einr}
  \begin{s-rem}
    ...
    constant invertible matrix $C_{\cY_{0,\tilde\theta}}\in\SSto_\theta(A^0)$.
  \end{s-rem}
\end{cor}
...
\begin{prop}\label{prop:filtrationOfStokesGroup}
  ...
  \begin{s-rem}\label{rem:filtrationOfStokesMats}
    Write $\rho_{\tilde\alpha}^k:\Sto^k_\alpha(A^0)\to\SSto^k_\alpha(A^0)$ for
    the \rewrite{restriction} of the map $\rho_{\tilde\alpha}$
    ...
    \[
      ...
      \SSto_\alpha(A^0)
      \overset{\cong}\longrightarrow
      \underset{k\in\cK_\alpha}\bigltimes\SSto_\alpha^k(A^0)
    \]
    ...
    \[ \begin{tikzcd}
        ...
        \dar{\rho_{\tilde\alpha}}
        & \prod_{k\in\cK_\alpha}\Sto_\alpha^k(A^0)\,,
        \dar{\prod_{k\in\cK}\rho_{\tilde\alpha}^k}
      \\\SSto_\alpha(A^0)
        \rar{i_\alpha^k}
...
\]
And since $\Sto_\alpha(A^0)$ has a faithful representation $\SSto_\alpha(A^0)$
we also get
\[
...
\begin{rem}\label{rem:mapStoToCocy}
  ...
  \PROBLEM[does not work for every set! needs some thm from boalch?]
...
\begin{defn}\label{defn:stokesCocycle}
  ...
  A \emph{Stokes cocycle} is a 1-cocycle\PROBLEM[only family?]
  $(\phi_\alpha)_{\alpha\in\A}$ of $\Lambda(A^0)$ which is
  \begin{itemize}
    \item indexed by the set of anti-Stokes directions and
    \item each component $\phi_\alpha$ determines an element of
      $\Sto_\alpha(A^0)$.
  \end{itemize}
  \begin{s-rem}
...
\begin{defn}
  ...
  \begin{comment}
    ...
    $H^1(S^1;\Lambda(A^0))$ or \emph{adequate} to $\Lambda(A^0)$ if for every
    element in
    $\underset{\cU}{\underrightarrow{\lim}}H^1(\cU;\Lambda(A^0))$
    given by some covering $\cU'$ and an element of $\Gamma(\cU';\Lambda(A^0))$
    there exists
    \begin{itemize}
      \item an element in $\Gamma(\cU;\Lambda(A^0))$ and
  ...
  \[
    \Gamma(\cU;\Lambda(A^0))\to\H^1(S^1;\Lambda(A^0))
  \]
...
\begin{lem}
  ...
  \begin{itemize}
    \item the canonical injective map
      \[
        i^k:\underset{\alpha\in\A}\prod\Sto_\alpha^k(A^0) \to
        \Gamma(\dot\cU^k;\Lambda^k(A^0)) \,,
      \]
    ...
    \item The map
      ...
      \begin{comment}
        ...
        \begin{itemize}
          ...
          \item See \cite[375]{Martinet1991}
        \end{itemize}
      \end{comment}
    \item The second map
      \[
