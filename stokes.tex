...
The meromorphic Connections are completely described by the Connection matrices
which lie in $\End(n,\C\{x\}[1/x])$\TODO.

\begin{defn}
  The set of \emph{applicable formal transformations}
  \[
    \hat G(A):=\left\{\hat F \mid [{}^{\hat F}\!A] \text{ is convergent}\right\}
    \subset \hat G
  \]
\end{defn}
\begin{comment}
