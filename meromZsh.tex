...
\begin{comment}
  \begin{itemize}
    \item \textbf{\cite[Prop.4.2.5]{Loday2014}}
...
\end{cor}
\begin{rem}
  We will see in lemma~\ref{lem:formSolToMat} that the matrix
  \[
    A={}^{\hat F}\left(t^LQ'(t^{-1})t^{-L}+L\frac{1}{t}\right)
  \]
  defines a system $[A]$ which has $\hat F t^L e^{Q(t^{-1})}$ as fundamental
  solution.
\end{rem}
