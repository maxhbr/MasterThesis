...
\begin{prop}
  \begin{comment}
    See \textbf{\cite[Rem.5.2.4]{hotta2008}} and \cite[Def.4.2.1]{Loday2014}
  \end{comment}
...
naturally associate \rewrite{the differential operator $\triangle=d-A$}, which
has order one and dimension $n$. \TODO[possibly multivalued
  solutions ($\tilde K$)?~\cite{hotta2008} on page 128]
...
\begin{prop}
  If we start with a system $[A]$ of rank $n$ we get a germ of a
  meromorphic connection via
  \[
    (\cM_A,\nabla_A)=(\C(\!\{t\}\!))^n,d-A)
  \]
...
\begin{defn}
  ...
  \begin{rem}
    This implies that, if $A$ has irregular/regular singularity, then also all
    meromorphic equivalent matrices ${}^{F}\!A$ have irregular/regular
    singularity.
  \end{rem}
\end{defn}
\begin{thm}
...
Fundamental solutions beschreiben also systems of ordinary differential
equations.
...
unique but all of them lie in the same convergent equivalence class.

On the other hand\TODO[same direction?], the Levelt-Turittin theorem says, that
each meromorphic connection is, after potentially needed ramification, formally
isomorphic to such a model, that means, that in each formal class there are
some models.
...
which is essentially a deduction from the Borel-Ritt lemma. \TODO{}
\begin{comment}
  We set 
  \[
  ...
  \begin{thm}[Sectorial decomposition]
    ...
    Let $(\cM,\nabla)$ be a meromorphic connection and let us assume that there
    exist a model $\cM^{model}$ together with an isomorphism
