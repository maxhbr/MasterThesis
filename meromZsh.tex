...
\begin{defn}
  \begin{enumerate}
    \item This matrix $A$ is called a \emph{connection matrix} of
      ...
      \begin{s-rem}
        We have seen, that a connection is fully determined by its connection
        matrix.
      \end{s-rem}
...
\end{defn}
\begin{lem}
  Let $(\cM^{model},\nabla^{model})$ be a model of a meromorphic connection,
  which is represented by the system $[A^0]$.
  \begin{enumerate}
    \item 
      \marginnote{\cite[Thm.4.3.1]{Loday2014}}
      If $(\cM^{model},\nabla^{model})$ is a model, if and only if there is a
      basis in which the fundamental solution\footnote{A $n\times n$ matrix
      whose columns are $n$ $\C$-linearly independent solutions of the system
      ...
      \begin{itemize}
        \item
          ...
          \begin{itemize}
            ...
            \item the $r_j$ are \TODO{} and satisfy therefore $\sum n_j=n$.
          \end{itemize}
          This means, that $Q=\diag(\underset{n_1\text{-times}}{\underbrace{%
            q_1,\dots,q_1}},q_2,\dots,q_s)$.
          \TODO[\textbf{ramified case}]
      ...
      \end{itemize}
      The matrix $\mathcal{Y}_0$ is \rewrite{called a \emph{normal solution}.}
      \begin{s-rem}
        In the unramified case $[Q,L]$, i.e.\ $Q$ and $L$ commute and $L$ can
        be supposed in Jordan form (cf.  \cite[Sec.4]{Martinet1991}).
      \end{s-rem}
      \begin{s-prop}
        A system $[A]$ is a model, if it has, up to meromorphic Abase change,
        has a fundamental solution of the form $t^L e^{Q(t^{-1})}$, i.e.\ it
        has a fundamental solution of the form
        \[
          Ft^L e^{Q(t^{-1})}
        \]
        with $F\in G(\!\{t\}\!)$.
    ...
    \item If $(\cM',\nabla')$ is obtained from $(\cM^{model},\nabla^{model})$
      via the formal meromorphic transformation $\hat F$, then is
      \[
        \mathcal{Y}=\hat F t^L e^{Q(t^{-1})}
      \]
      a fundamental solution for $Y'={}^{\hat F}\!A^0(t)Y$.
      \begin{s-rem}
        It is allways possible to permutate the collums of a fundamental
        solution by 
        \[
          \mathcal{Y}=\hat F t^{P^{-1}LP} e^{P^{-1}Q(t^{-1})P}
        \]
...
\TODO[move] \TODO[maby \textbf{not necessary}, since we have Borel-Ritt!]
\begin{comment}
  \begin{multicols}{2}
    \textbf{Classical:}
    \begin{itemize}
...
\begin{rem}
  We will use this in the case, when $A$ ist via $\hat F$ formally equivalent
  to $A^0$. We are then able to find a \rewrite{(cyclic)} covering of the $S^1$
  of arcs such that on every arc there exists a lift $\tilde F$ of $\hat F$.
\end{rem}
\TODO{}
\begin{comment}
  \begin{multicols}{2}
  ...
  \columnbreak
    ...
    \begin{thm}[Sectorial decomposition]
      ...
      there exist a model $\cM^{model}$ together with an isomorphism
      $\hat\lambda:\hat\cM\to\hat\cM^{model}$. There exists then, for any
      $e^{i\theta^0}\in S^1$, an isomorphism $\tilde\lambda_{\theta^0}:
      \tilde\cM_{\theta^0}\to\tilde\cM_{\theta^0}^{model}$
      lifting $\hat\lambda$, that is, such that the following diagram
      \[ \begin{tikzcd}
          \tilde\cM_{\theta^o} \dar \rar{\tilde\lambda_{\theta^o}} &
          \tilde\cM^{model}_{\theta^o} \dar
...
isomorphic to $(\cM^{model},\nabla^{model})$. This is the set, that we will be
