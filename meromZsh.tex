...
\begin{defn}\label{defn:normSol}
  Let $[A]$ be a system. If its fundamental solution has the form
  \[
  ...
  \begin{itemize}
    ...
    \item $L\in\gl_n(\C)$ constant matrix called the \emph{matrix of formal
      ...
      this generic enough?}
    \item $F$ a transformation,
  \end{itemize}
  will be called a \emph{normal form}. The normal forms will often be
  denoted $A^0$.
  The fundamental solution $F t^L e^{Q(t^{-1})}$ will, in this case, be called
  \emph{normal solution} and will often be denoted by $\cY_0$.
  \begin{s-rem}
    By changing the basis via $F^{-1}$ wo obtain from $[A^0]$ the equivalent
