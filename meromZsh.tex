...
We will use \rewrite{all of the} mentioned \rewrite{resources}.
...
\begin{comment}
  \begin{lem}
    \PROBLEM[how looks the connection matrix? Simly the sum?]
    Let $(\cM_1,\nabla_1)$ and $(\cM_2,\nabla_2)$ be meromorphic connections
...
\begin{prop}\label{prop:fundSolBuilder}
  ...
  \]
  where $Q$ and $L$ are as above, i.e.\ they satisfy
  $t^LQ'(t^{-1})t^{-L}=Q'(t^{-1})$.
\end{prop}
\begin{proof}
  ...
  \\&\overset{\star}{=}\left(Q'(t^{-1})+L\frac{1}{t}\right)t^Le^{Q(t^{-1})}
  \\&=A^0\cY_0 \,.
  \end{align*}
  The equality, marked by the $\star$, is satisfied, since the block structure
  of $L$ is finer then the structure of $Q(t^{-1})$, and thus the structure of
  $Q'(t^{-1})$. Thus we can look at the problem
  $t^LQ'(t^{-1})t^{-L}=Q'(t^{-1})$ blockwise, where $Q'(t^{-1})$ can be
  written as $q'(t^{-1})\cdot\id$ for some $q'(t^{-1})$. The equation is then
  clearly satisfied.
...
becomes the following (cf.~\cite[Thm.II.5.12]{sabbah2007isomonodromic}
or~\cite[Sec.II.2.4]{sabbah_cimpa90}).
...
\marginnote{\cite[6]{thboalch}
  ...
  \\\cite{babbitt1983}}
It is convenient to look at the slightly larger space of \emph{marked
(meromorphic) pairs}
\[
  \cH=\cH(\cM^{nf},\nabla^{nf})=\left\{
    \left[(\cM,\nabla,\hat f)\right]
...
elements are block-diagonal\TODO[corresponding to the structure of $Q$],
