...
\begin{defn}\label{defn:mercon}
  ...
  \begin{s-rem}
    ...
    defined in~\cite[Sec.0.8]{sabbah2007isomonodromic} and $\Omega_M^1(*Z)$ is
    then defined as
    \[
...
\begin{rem}
  \begin{enumerate}
    ...
    \item Here, the variant `holomorphic bundle with meromorphic connection' is
      ...
      meromorphic connection in our sense.
  \end{enumerate}
\end{rem}
...
$\nabla s=0$, correspond to solutions of
...
\begin{rem}
  ...
  \begin{s-rem}
    ...
    basis choices $\cM\overset{\sim}\longrightarrow\C(\!\{t\}\!)^n$ and
    $\cM'\overset{\sim}\longrightarrow\C(\!\{t\}\!)^n$ we have the following
    commutative diagram:
    \[ \begin{tikzcd}
        \C(\!\{t\}\!)^n \arrow{rrrr}{F}\arrow{ddd}{d-B}&&&& \C(\!\{t\}\!)^n\arrow{ddd}{d-A}
        \\ & \cM \dar{\nabla}\arrow{rr}{\Phi}\ular&& \cM'\dar{\nabla'}\urar
        \\ & \cM \arrow{rr}{\Phi}\dlar&& \cM'\drar
        \\ \C(\!\{t\}\!)^n \arrow{rrrr}{F}&&&& \C(\!\{t\}\!)^n
    \end{tikzcd} \]
