...
\subsection{Meromorphic / formal transformation II}
\begin{defn}
  \begin{itemize}
...
Let us now fix a model $\cM^{good}$.
\begin{comment}
  \begin{itemize}
...
Our goal is to understand the set
\begin{align*}
  {}^0C(\cM^{good},\nabla^{good})=\left\{
    \left[(\cM,\nabla)\right]
  ...
  \right\}
\end{align*}
of all isomorphism classes meromophic connections, which are formally
...
Since the meromophic connections are described by the connection matrices we
will \TODO{}

\dots thus the set of meromophic classes of systems can be identified to the
left and right quotient
\[
  G\backslash\hat G(A^0)/G_0(A^0)
\]
whereas the set of meromophic classes of transformations of $[A^0]$ is
represented by the left quotient
\[
...
\begin{comment}
and
$(\cM,\nabla)$ is (formally) isomorphic to $(\cM',\nabla')$ if and only if
there exists a (formal) transformation $F$ such that \TODO, we get
\begin{notations}
  \begin{itemize}
    \item Denote the set of all systems, which are formally equivalent to
      $d-A^0$ by
      \[
        \Syst(A^0)=\{d-A
    ...
    \item
      \[
        \hat\Syst(A^0)=\{(A,\hat F)
          \mid A={}^{\hat F}\!A^0 \text{ for some } \hat F\in G(\!(t)\!)\}
      \]
  \end{itemize}
  \TODO[analytic or meromorphic classification?]
\end{notations}

\begin{lem}
  \marginnote{see \cite[854]{Loday1994}}
  \begin{itemize}
    \item The left and right quotient
      \[
        G\backslash\hat G(A^0)/G_0(A^0)
      \]
      of
    ...
    \item
      \[
        G\backslash\hat G(A^0)
      \]
      represets the set of meromophic classes of transformations of $[A^0]$.
  \end{itemize}
\end{lem}
\end{comment}
