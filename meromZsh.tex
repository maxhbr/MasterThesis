...
\begin{proof}
  ...
  normal solution. We use proposition~\ref{prop:fundSolBuilder} to write the
  connection matrix as
  \[
    A=t^LQ'(t^{-1})t^{-L}+L\frac{1}{t}
  \]
  and the proposition~\ref{prop:systToMeromConn} yields our meromorphic
  connection $(\cM_A,\nabla_A)=(\C(\!\{t\}\!)^n,d-A)$ for \rewrite{which we
  want to prove, to be} a model.
  \TODO{}
\end{proof}
\begin{cor}
