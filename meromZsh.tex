...
\begin{prop}
  \begin{comment}
    See 
    \begin{itemize}
      \item \textbf{\cite[Rem.5.2.4]{hotta2008}}
      \item \cite[Def.4.2.1]{Loday2014}
    \end{itemize}
  \end{comment}
...
naturally associate \rewrite{the differential operator $\nabla=[A]=d-A$}, which
...
\begin{defn}
  \begin{enumerate}
    ...
    \item A \emph{germ of a meromorphic linear differential system} of rank
      ...
      \begin{comment}
        \cite{Loday1994} and \cite{Loday2014} calls them differential modules?
      \end{comment}
  \end{enumerate}
\end{defn}
...
\begin{notations}
  ...
  to~\cite{boalch,thboalch} where formal classification is used.
\end{notations}
