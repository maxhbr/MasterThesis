...
If we want to reverse the base choice to get a corresponding meromorphic
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \TODO[Maybe see: \cite{sibuya1990Linear} Thm.3.3.1: G. D. Birkhoff]
  \begin{thm}
    \TODO[Quelle?]
  ...
  \begin{itemize}
    ...
    \item Let $G_0(A^0)$ denote the set of all isotropies of $A^0$.
      \begin{s-rem}
        ...
        $G(A^0)$ is a subgroup of $\Gl_n(\C[1/x,x])$.
      \end{s-rem}
  \end{itemize}
\end{defn}
\begin{lem}
...
\begin{defn}
  ...
  A \emph{fundamental matrix of solutios} or \emph{fundamental solution} $\cY$
  an a sector $\mathfrak{s} $\PROBLEM[?] of the system $[A]$ is an invertible
  $n\times n$ matrix, which solves $[0,A]$ on $\mathfrak{s}$.
  \begin{s-rem}
...
\begin{defn}
  ...
  \]
  where $L\in G$ is called the \emph{monodromy matrix} of $[A]$.
  \TODO[wie passt das zu normal-formen?]
\end{defn}
...
\begin{defn}\label{defn:elemnMerConnBausteine}
  \begin{itemize}
    ...
    \item For $\alpha\in\C$, define the \emph{elementary regular meromorphic
      ...
      corresponding connection by $\cN_{\alpha,d}$\comm{~where $d$ is the
      dimension minus one}.
  \end{itemize}
\end{defn}
\begin{prop}
  \begin{enumerate}
    \item
      ...
      \begin{s-rem}
        ...
        the book~\cite[Sec.5.2]{hotta2008}.
      \end{s-rem}
...
\begin{defn}\label{defn:normSol}
  ...
  \begin{s-rem}
    \TODO[Cor from base change for fundamental solutions] By changing the basis
...
\begin{proof}
  ...
  and $\frac{d}{dt}t^L=L\frac{1}{t}t^L$, thus we can prove that $\cY$ is a
...
\begin{proof}
  ...
  \begin{comment}
    ...
    \]
  \end{comment}
  Since $\cM$ is a sum of sach elementary connections, The connection matrix of
  $\cM$ writes as
  \[
    \bigoplus_\phi\left(
      -\phi'\id + \frac{1}{t} \bigoplus_{(\alpha,N)}\left( \alpha\id+N\right)
...
\begin{rem}
  ...
  (cf.~\cite{thboalch}\TODO[better:~\cite{babbitt1989local}]).
