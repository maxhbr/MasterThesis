  ...
  \begin{center}
    \begin{tikzpicture}[scale=4]
      ...
      \draw [blue!40!black,dashed]
        ({\argA+4/\k + 1/2/\k},{-\absA - 0.2}) -- ({\argA+4/\k + 1/2/\k},0);
      \draw [blue!40!black,dashed]
        ({\argA+4/\k + 3/2/\k},{-\absA - 0.2}) -- ({\argA+4/\k + 3/2/\k},0);
      \draw [blue!40!black, thick
            ,decorate
            ,decoration={brace,mirror,amplitude=10pt}
            ,xshift=0pt
            ,yshift=0pt]
        ({\argA+4/\k + 1/2/\k},{-\absA - 0.2}) -- ({\argA+4/\k + 3/2/\k},{-\absA - 0.2})
...
\end{rem}
\begin{comment}
  Let us now use the defined relations from
  Definition~\ref{defn:definingRelations} to say, which are the interesting
  directions of $S^1$.
\end{comment}
\begin{defn}\label{defn:antiStokesDir}
  ...
  \end{enumerate}
\end{defn}
If one starts with a fundamental solution on a sector, one is able to extend
this analytic to an larger sector up to some barrier.
The Stokes directions are exactly the directions, beyond which some fundamental
directions can't be extended.
...
$\Lambda(A^0)$, which will be restricted to $\Sto_\theta(A^0)$.}
...
\end{rem}
We say that \emph{$\alpha$ is bearing the level $k$} if $\alpha\in\A^k$ and
denote the \emph{set of levels beared by an direction $\alpha\in\A$} by
\[
...
\begin{prop}\label{prop:PropertiesOfStokesSheafSplitting}
  ...
  For any level $k\in\cK$ one has that
  \begin{enumerate}
  \item $\Lambda^{k}(A^0)$, $\Lambda^{\leq k}(A^0)$ and $\Lambda^{<k}(A^0)$ are
    sheaves of subgroups of $\Lambda(A^0)$,
  \item the sheaf $\Lambda^k(A^0)$ is normal in
    $\Lambda^{\leq k}(A^0)$ and
  \item
    \marginnote{\cite[Proposition 10]{Martinet1991}}
    if one defines
    \begin{itemize}
    \item $i:\Lambda^k(A^0)\hookrightarrow\Lambda^{\leq k}(A^0)$ to be the
      canonical inclusion and
    \item $p:\Lambda^{\leq k}(A^0)\twoheadrightarrow\Lambda^{<k}(A^0)$ to be the
      truncation to terms of levels $<k$
    \end{itemize}
    does the exact sequence of sheaves
    \[
...
\begin{cor}\label{cor:factorStokesGerms}
  ...
  \begin{s-rem}\label{rem:algFactorization}
    ...
    keeping in the representations $1+\sum C^{(j,l)}$ of $f^{\leq k}$ only the
...
\end{prop}
  % \begin{s-rem}\label{rem:filtrationOfStokesMats}
    ...
    By abuse of notation we denote the induced decomposition also by
    \[
      i_\alpha:
      \SSto_\alpha(A^0)
      \overset{\cong}\longrightarrow
