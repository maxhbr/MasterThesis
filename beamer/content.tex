...
\begin{frame}[t,fragile]{Keim eines Meromorphen Zusammenhangs}
  ...
  \only<2>{%
    ...
    \begin{itemize}
      \item einen $\C\{t\}$-Vektorraum $~~\tikzmark{e1}\!\!\!\!\cM$ und
        \begin{flushright}
          \tikzmarkc{n1}{gray}
          \textcolor{gray}{Keim von $\sM$}
        \end{flushright}
        \begin{tikzpicture}[remember picture,overlay]
          \draw[->,gray!50!white,thick] (n1) to[out=180,in=-90] (e1);
        \end{tikzpicture}
      \item einer $\C$-linearen Derivation $\nabla:\cM\to\cO(*Z)\otimes\cM$
        welche
        \begin{itemize}
          \item die \emph{Leibnitz-Regel}
            \begin{multicols}{2}
              \[
...
\begin{frame}[t,fragile]{Borel-Ritt}
  ...
  \only<2>{%
    \begin{flushright}
      \tikzmarkc{n1}{gray}
      \textcolor{gray!50!white}{$t\mapsto e^{-\frac{1}{t}}
