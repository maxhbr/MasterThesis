...
%         % $\nabla:\cM\to\C\{t\}[t^{-1}]\otimes\cM$
%         $\nabla$ welche die \emph{Leibniz Regel}
%         \begin{multicols}{2}
%           \[
%             \nabla(\tikzmark{e2}\myf\overset{\tikzmark{e1}}\mym)
%             =\frac{\partial\myf}{\partial t}\mym+\myf\nabla\mym
%           \]
%           \columnbreak
%           \begin{itemize}
%             \item[\tikzmarkc{n1}{blue}] $\mym\in\cM$
%             \item[\tikzmarkc{n2}{red}] $\myf\in\C\{t\}$
%           \end{itemize}
%           \begin{tikzpicture}[remember picture,overlay]
%             \draw[->,blue!50!white,thick] (n1) to[out=180,in=70] (e1);
%             \draw[->,red!50!white,thick] (n2) to[out=180,in=-70] (e2);
...
\begin{frame}[t]{Zusammenhangs Matrix}
  Zu einer Basis $\textbf{e}$ einer lokalen Trivialisierung von $\sM$ gibt es
  eine \emph{Zusammenhangs Matrix} $A=A'dt$ für ein $A'\in M(n\times
...
\begin{frame}{Matrix Version: Definitionen}
  \begin{multicols}{3}
    ...
    \only<2| handout:0>{%
      \begin{tikzpicture}[scale=3,remember picture]
        \node[] (e3) at ({cos(70)},{sin(70)}) {};
        % \node[] (e3b) at ({cos(310)},{sin(310)}) {};
        \node[] (zero) at (0,0) {};
        ...
        {\draw[thick,purple!\w!blue,path fading=west]
         ...
         {\draw[green!20!white,thick] (zero) -- +({cos( \sep )},{sin( \sep )});
          ...
          \fill[blue!20!white] ({cos( \w + \sep )},{sin( \w + \sep )}) circle
          (.7pt);
         }
        };
    ...
    \columnbreak
      \begin{itemize}
        \item<2>[\tikzmarkc{n3}{gray}] \textcolor{gray}{durch
          $\frac{\pi}{k-1}$-rotations Symmetrie}
        \item<3>[\tikzmarkc{n2}{red}] $l:=r/(2k-2)$
        \item<3>[\tikzmarkc{n1}{blue}] $r:=\#\A$
        % \item $\textbf{\underline{d}}:=(d_1,\dots,d_l)$
      \end{itemize}
    \visible<2>{%
      \begin{tikzpicture}[remember picture,overlay]
        \draw[->,gray!50!white,thick] (n3) to[in=70,out=180] (e3);
      \end{tikzpicture}
    }
    \visible<3->{%
      \begin{tikzpicture}[remember picture,overlay]
  ...
  \begin{thm}
    \begin{itemize}
      \item Zu $(A,\hat F)$ gibt es auf jedem Sektor
        $\Sect\left(d_i,d_{i+1}\right)$ einen kanonischen Lift
        \[
          \Sigma_i(\hat F)
