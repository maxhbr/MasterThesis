...
Here we will look at the set $\cH(A^0)$ of isomorphism classes of marked pairs
and we will proof that it is isomorphic \TODO[as\dots] to the first non abelian
cohomology set $H^1(S^1;\Lambda(A^0))$ which will be denoted as $\St(A^0)$.  If
we talk about cocycles or cochains, we will in the following always mean
...
\begin{defn}\label{defn:StokesSheaf}
  \begin{comment}
    ...
    \begin{enumerate}
    \item Flatness:
      $\underset{x\in\mathfrak{s}}{\underset{x\to0}{\lim}}f(x)=\id$ and
      $f\sim_{\mathfrak{s}} 1$;
  ...
  \item Multiplicatively flatness: $f$ is asymptotic to the identity,
    i.e.\ $f\sim_{\mathfrak{s}} 1$ for some $\mathfrak{s}$ containing $\theta$;
  \item Isotropy of $A^0$: ${}^f\!A^0=A^0$.
  \end{enumerate}
  \begin{s-rem}
...
Since the sheaf by definition only contains solutions of the differential
equation $[\End A^0]$ we obtain the following proposition.
\begin{prop}\label{prop:piecewiseConstant}
  The sheaf $\Lambda(A^0)$ is piecewise-constant
  (cf.~\cite[Prop.2.1]{Loday2004}).
\end{prop}

\subsection{The theorem}
\rewrite{Let us now state the Malgrange-Sibuya Theorem. We will first give it in
the language of meromorphic connections and after that we will give the same
theorem in the language of systems.
The second formulation of this theorem will be proven in the next section.}

In the language of meromorphic connections is the map, of the Malgrange-Sibuya
Theorem below, described as follows.
...
\begin{rem}\label{rem:expNonNormalForm}
  ...
  This is formulated in the following commutative diagram:
  \begin{center}
...
We will denote the set of all such holomorphic functions, which are
multiplicatively flat on $I$, i.e.\ are on the arc $I$ asymptotic to
$\id\in G(\!(t)\!)$, by
\[
...
\begin{einr}
  ...
  The Borel-Ritt Lemma yields for every arc $J\subsetneq S^1$ containing $j$ a
...
Ramis. It is also mentioned on page 30 of~\cite{Varadarajan96linearmeromorphic}.
