...
\begin{frame}{Systeme und die Gauge Transformation}
  \begin{defn}
    ...
    System (also: System linearer gewöhnlicher komplexer
    Differentialgleichungen)
    \[
      dX=AX \,.
    \]
  ...
  \uncover<2->{%
    \begin{prop}
      ...
      erhält man das System ${}^F\!A$, gegeben durch die
      \emph{Gauge-Transformation}
      \[
        {}^F\!A:=(dF)F^{-1} + FAF^{-1} \,.
      \]
...
\begin{frame}{Die Normalform und formale Klassifikation}
  \uncover<2->{%
    \begin{defn}
      Eine \emph{Normalform} ist einem System äquivalent zu einem System der
      Form
      \[
  ...
  \uncover<3->{%
    ...
    \begin{cor}
      ...
      zu einer Normalform und Normalformen sind bis auf (konvergente)
...
\begin{frame}{Die klassifizierende Menge}
  ...
  \uncover<2->{%
    ...
    Betrachte dazu die folgende Menge:
    \begin{defn}
      Die \emph{klassifizierende Menge} ist definiert als die Menge
      \[
        \cH(A^0):=\left\{\left[\!\bigl(A,\hat F\bigl)\!\right]
          \mid A={}^{\hat F}\!A^0 \text{ mit } \hat F\in G(\!(t)\!)\right\}
        \raisebox{0.25em}{\tikzmark{n1}}
      \]
      der Äquivalenzklassen \emph{markierter Paare} zu $A^0$.
    \end{defn}
  }
  \uncover<3->{%
    \begin{tikzpicture}[remember picture,overlay]
      \draw[->,blue!50!white,thick,dotted] (n1)
        to[out=0,in=0]
        node[fill=white]{einfach} (e1);
    \end{tikzpicture}
  }
\end{frame}
\section{Die Stokes Strukturen}
\iffalse
...
\begin{frame}[fragile]{Malgrange-Sibuya-Isomorphismus}
  Zu einem $(A,\hat F)$ wähle sektorweise asymptotische Lifts
  $F_j\in\Lambda(A^0)(U_j)$ der formalen Transformation $\hat F$.
...
\section*{Fragen?}
\only<beamer>{\setbeamercolor{background canvas}{bg=black}\begin{frame}
  \end{frame}\setbeamercolor{background canvas}{bg=}}
...
\begin{frame}[t,fragile]{Die anti-Stokes Richtungen}
  \begin{tikzpicture}[scale=4,transform canvas={yshift = -4.5cm}, remember picture]
...

    {
      \clip (0,0) circle (1cm);
      \only<1-2>{
        \fill[green!50!black] ({cos( \t )},{sin( \t )}) circle (.8pt);
      }
      \only<2>{
        \fill[green!50!black] ({cos( 0 )},{sin( 0 )}) circle (.8pt);
      }
      \uncover<3->{%
    ...
    \node (e3z) at ({cos( 40 ) * 1.03},{sin( 40 ) * 1.03}) {};
    \node (e3a) at ({cos( \t ) * 1.03},{sin( \t ) * 1.03}) {};
    \node (e3b) at ({cos( 0 ) * 1.03},{sin( 0 ) * 1.03}) {};
    \node (e3c) at ({cos( -10 ) * 1.03},{sin( -10 ) * 1.03}) {};
    \node (e3d) at ({cos( -20 ) * 1.03},{sin( -20 ) * 1.03}) {};

    \uncover<2>{%
      \draw[dotted,->,green!50!black,thick] (e3a) to[out=200,in=180] node[xshift=5pt] {$\frac{\pi}{k}$} ([xshift=-1.8pt]e3b);
    }
    \uncover<3->{%
      \draw[dotted,->,green!50!black,thick] (e3a) to[out=200,in=180] ([xshift=-1.8pt]e3b);
      \draw[dotted,->,green!50!black,thick] (e3a) to[out=200,in=220] ([xshift=-1.4pt,yshift=-1.1pt]e3z);
      \draw[dotted,->,brown,thick] (e3a) to[out=200,in=170] ([xshift=-1.1pt,yshift=.2pt]e3c);
      \draw[dotted,->,green!50!black,thick] (e3a) to[out=200,in=160] ([xshift=-1.6pt,yshift=.7pt]e3d);
    }
  \end{tikzpicture}
  \vspace{-1em}
  \begin{minipage}{1\textwidth}
    \begin{flushright}
      \begin{minipage}{.5\textwidth}
        \begin{defn}
          Zu jeder Richtung $\theta$ definieren wir \emph{Menge der
            dazugehörigen Level \boldmath$\cK_\theta$} als die $k\in\N$ für die
          \begin{itemize}
          \item[] für jedes $i\in\N$ gilt, dass
            \[
              \theta+i\frac{\pi}{k} \in \A \,.
            \]
          \end{itemize}
        \end{defn}
        \uncover<4->{%
          \begin{lem}
            Gilt für jedes $\theta\in\A$ dass $\cK_\theta=\{k\}$, so haben die
            anti-Stokes Richtungen eine $\frac{\pi}{k}$-drehsymmetrie.
          \end{lem}
        }
      \end{minipage}
    \end{flushright}
...
\begin{frame}{Wann ist $f\in\Lambda_\theta(A^0)$ von maximal decay?}
  Ein Keim $f\in\Gl_n(\cA_\theta)$, der eine Isotropie von $A^0$ ist, sieht wie
  folgt aus
  \begin{align*}
    f  &=
         \only<beamer>{
