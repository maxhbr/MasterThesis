...
\subsubsection{What follows for the Stokes germs $\phi_\theta$?}
Look, as an example, at the case, where
\begin{itemize}
  \item $\cK_{\theta}=\cK$,
...
\[
  ...
  \end{pmatrix}
\]
which gets, via $\rho_{\tilde\theta}^{-1}$, mapped to
\begin{align*}
  \rho_{\tilde\theta}^{-1} \left(C_{\cY_{0,\tilde\theta}}\right)
  &=t^{L}e^{Q(t^{-1})}C_{\cY_{0,\tilde\theta}} e^{-Q(t^{-1})}t^{-L}
\\&=t^{L}e^{Q(t^{-1})}
  \begin{pmatrix}
    1     & \star & \star
...
\\&=
  \begin{pmatrix}
    1
    & t^{L_{1,1}}e^{q_1(t^{-1})}\star t^{-L_{2,2}}e^{-q_2(t^{-1})}
    & t^{L_{1,1}}e^{q_1(t^{-1})}\star t^{-L_{3,3}}e^{-q_3(t^{-1})}
    \\0     & 1     & 0
    \\0
    & t^{L_{3,3}}e^{q_3(t^{-1})}\star t^{-L_{2,2}}e^{-q_2(t^{-1})}
    & 1
  \end{pmatrix}
  \in\Sto_\theta(A^0)
\end{align*}
Thus, for every determination $\tilde\theta$ of $\theta$ we have an isomorphism
\begin{align*}
  \C^3 &\overset\cong\to \Sto_\theta(A^0)
\\(c_1,c_2,c_3) &\mapsto
  \begin{pmatrix}
    1 & t^{L_{1,1}}e^{q_1(t^{-1})}c_1 t^{-L_{2,2}}e^{-q_2(t^{-1})}
    & t^{L_{1,1}}e^{q_1(t^{-1})}c_2 t^{-L_{3,3}}e^{-q_3(t^{-1})}
    \\0     & 1     & 0
    \\0 & t^{L_{3,3}}e^{q_3(t^{-1})}c_3 t^{-L_{2,2}}e^{-q_2(t^{-1})} & 1
  \end{pmatrix}
\end{align*}
