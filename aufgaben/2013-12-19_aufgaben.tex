...
\end{tikzcd}
\]
Diese Sequenz splitet\comm{(Quelle?)}, da $G$ in $G_k$ als die Untergruppe der
konstanten Matritzen enthalten ist und die entsprechende Verknüpfung der
Morphismen der Identität $\Id_G$ entspricht.
Es folgt\comm{(Quelle?)}, dass $G_k$ das semi-direkte Produkt $G\ltimes B_k$
...
Ein Element $G$ aus $G_k$ hat die Form $A=A_0+A_1\xi+\dots+A_{k-1}\xi^{k-1}$,
...
Denn $\det(\sum^{k-1}_{i=0}A_{i}\xi^{i})=\overset{\text{\color{red}?}}\dots
=\det(A_0)$.\\
...
\begin{comment}
\begin{defn}
\dots \emph{schöne koadjungierte Orbiten} \dots
\end{defn}
...
\end{comment}
Dies identifiziert den Quotienten $\mu^{-1}(0)/B_k$ mid $\tilde O$ als
Mannigfaltigkeit.
\begin{comment}
???
\end{comment}
