...
wird. Beispielsweise $\footnotesize\begin{pmatrix}a_{i,0} & 0\\ a_{i,1} & 1 \end{pmatrix}$,
diese schickt $[1:0]$ auf $a_i$.
...
Ein Element $A$ aus $G_k$ hat die Form $A=A_0+A_1\xi+\cdots+A_{k-1}\xi^{k-1}$,
...
Sei $A\in O_B$, sei $g\in \Gl_n(\C)$ dann ist $(\id,A)$ und $(g^{-1},gAg^{-1})$
in $\tilde O$.\\
Es sind für alle $g$
\[
O_B \hookrightarrow \tilde O; \qquad A \mapsto (\id,A)
\qquad\text{sowie}\qquad
O_B \hookrightarrow \tilde O; \qquad A \mapsto ((g^{-1},gAg^{-1}))
\]
Homöomorphismen auf ihr Bild. \comm{Klar? Nachweis?}\\
Es ist für jedes $A$ auch
\[
\Gl_n(\C) \hookrightarrow \tilde O; \qquad g \mapsto ((g,g^{-1}Ag))
\qquad\text{bzw.}\qquad
\Gl_n(\C) \hookrightarrow \tilde O; \qquad g \mapsto ((g^{-1},gAg^{-1}))
\]
