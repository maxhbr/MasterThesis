...
Define
\[
  \exp_{\mu_1}(\hat F):G\backslash\hat G(A^1) \to H^1(S^1;\Lambda(A^1))
\]
as the Malgrange-Sibuya isomorphism in the case, where $A^1$ is not generally
in normal form. \TODO[$\exp_{\mu_0}$?? generic definition?]
...
\begin{defn}
  ...
  $\exp_{\mu_1}(\hat F)$ contains a $k$-summable cocycle.
\end{defn}
...
\begin{defn}\label{defn:singlDir}
  ...
  $k$-summable cocycle in $\exp_{\mu_1}(\hat F)$.
...
$\dot f^{<k}$ belongs to $\exp_{\mu_0}(\hat F^{<k})$.
...
\begin{proof}
  ...
  \[
    \exp_{\mu_0}(\hat H^{<k})=\exp_{\mu_0}(\hat F^{<k}).
  \]
\end{proof}
...
\begin{comment}
  \begin{cor}
    ...
    Write the twisted cocyle $\dot\phi\in\exp_{\mu_1}(\hat F^k)$ in terms of
