...
We will mostly refer to Loday-Richaud's book~\cite[chapter 2]{Loday2014} and
the book~\cite[chapter 7]{van2003galois} written by van der Put and Singer
although this topic is scratched in many publications.
...
\begin{rem}
  \begin{itemize}
    ...
    \item Since the explicit value $r$ of the radius does not matter as long as
      it is small enough, we shall simply speak of an \emph{arc} or
      \emph{interval} $U(\theta,\epsilon):=\left(\theta-\frac{\epsilon}{2}
      ...
      \TODO[Define via limit?]
    \item We will denote the sector defined by an arc $I$ as
      $\mathfrak{s}_I:=\mathfrak{s}_I(r)$ for small enough $r$.
  \end{itemize}
\end{rem}
\begin{defn}
...
book~\cite{Loday2014}. \TODO[we will scratch\dots in section?]
...
\begin{comment}
  ...
  surjective.
  \begin{defn}[Gevrey asymptotics (of level $k$)]
    \marginnote{\cite[Defn.2.3.1]{Loday2014}}
    A series $\sum_{n\geq0}a_nt^n$ is of level $k$\PROBLEM[$k=1/s$?] if there
    exist constants $C>0$, $A>0$ such that the coefficients $a_n$ satisfy
    \[
      |a_n|\leq C(n!)^sA^n \qquad\text{for all $n$.}
    \]
  \end{defn}
