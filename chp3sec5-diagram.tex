...
\begin{center}
  \begin{tikzpicture}
    ...
    \node (cH2) at ([yshift=-\dy]cH) {$G(\!\{t\}\!)\backslash\hat G(A^0)$};
    ...
    \draw[->] (cH2) -- (cH)
      node[midway,right] {Cor.~\ref{cor:isomorphyOfClassfset}};
    \draw[->] (cH2) -- (ProdOfStos)
      node[midway,left] {$g$};
    \draw[->] (cH) -- (StA0)
    ...
    \draw[->] (SSS3P) -- (ProdOfSStos)
      node[midway,below left] {$\prod_{\alpha\in\A}\vartheta_\alpha$};
  \end{tikzpicture}
\end{center}
Where
\begin{itemize}
