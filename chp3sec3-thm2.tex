...
\begin{enumerate}
  \setcounter{enumi}{0}
\item The first covering $\cU^{k}=\{\dot U_\alpha^k\mid\alpha\in\A^k\}$ is the
  cyclic covering determined by the nerve
  \[
    \dot\cU^k:=
...
$\#J>1$, the set
\[
  \bigcup_{k\in J}\left\{U(\alpha,\frac{\pi}{k})\mid\alpha\in\A^{k}\right\}
...
\begin{proof}
  ...
  \item The map
    ...
    \begin{itemize}
    \item the Proposition~\ref{prop:piecewiseConstant} implies that the sections
      of $\Lambda(A^0)$ on arcs are uniquely determined by their germs and
    ...
    In~\cite[5269]{Loday2004} is also stated, that only the Stokes
    germs at $\alpha$ are sections of $\Lambda(A^0)$ on $\dot\cU_\alpha^k$ and
